  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 md:px-6 md:py-14">
      ...
      {checkoutDemo && mode === "demo" ? (
        <p
          className="mt-6 border border-border bg-paper px-4 py-3 text-sm"
          role="status"
        >
          Checkout is fulfilled by Shopify. This catalogue is running in demo mode, so payments are
          not taken. Connect a store to enable hosted checkout.
        </p>
      ) : null}
      {lines.length === 0 ? (
        <EmptyState
        ...
        <div className="mt-10 grid gap-12 lg:grid-cols-[minmax(0,1fr)_280px]">
          ...
          <aside className="h-fit space-y-4 border border-border bg-paper p-6">
            <div className="flex justify-between text-sm">
              <span>Subtotal</span>
              <span>{cart ? formatMoney(cart.cost.subtotalAmount) : null}</span>
            </div>
            ...
            <p className="text-xs text-muted">
              ...
              until the threshold is met. Duties are not included.
            </p>
