...
3. **Products → Inventory** — Headless carts reserve stock. If Chrome `POST /api/cart` returns `{"ok":false,"code":"out_of_stock"}` while the PDP still says in stock, raise available quantity or enable **Continue selling when out of stock**. The page is cached; the cart call is live.
4. Optional: add Finnish (and Swedish) catalogue translations in Shopify. Until those exist, `/fi` uses repository overlays for known Paw & Pine handles. Brand names stay as designed.
